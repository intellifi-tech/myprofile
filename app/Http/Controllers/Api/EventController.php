...
class EventController extends Controller
{
    public function index(Request $request)
    {
        if ($request->header('api-token')) {
            ...
            if ($user) {
                $events = Event::orderBy('created_at', 'DESC')->with(['userAttendedEvent', 'userAttendedEvent.user'])->limit(100)->get();
                if ($events->count() > 0){
                    $json['status'] = 200;
    ...
    public function show(Request $request, $event_id)
    {
        if ($request->header('api-token')) {
            ...
            if ($user) {
                if ($event_id) {
                    $event = Event::where('id', $event_id)->with(['userAttendedEvent', 'userAttendedEvent.user', 'userAttendedEvent.comments'])->first();
                    if (!is_null($event)){
