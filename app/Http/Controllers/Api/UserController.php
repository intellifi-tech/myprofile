                ...
                if ($user){
                    ...
                    if ($check == false){
                        ...
                        return response()->json([
                            ...
                            'api_token' => $user->api_token,
                            'id' => $user->id,
                            'profile_photo' => "https://demo.intellifi.tech/demo/MyProfile/web/public/uploads/profile/".$user->profile_photo,
                            'cover_photo' => "https://demo.intellifi.tech/demo/MyProfile/web/public/uploads/cover/".$user->cover_photo,
                            'title' => $user->title,
                            'name' => $user->name,
                            'surname' => $user->surname,
                            'career_history' => $user->career_history,
        ...
        if ($request->header('api-token')) {
            ...
            if ($user) {
                if ($request->title && $request->latitude && $request->latitude && $request->event_image) {
                    ...
                    $json['userAttendedEvent']['event'] = event_image_path() . $userAttendedEvent->event_image;
                    return response()->json($json, 200, [], JSON_UNESCAPED_UNICODE);
                } else {
                    $json['status'] = 204;
